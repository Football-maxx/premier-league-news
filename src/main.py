...
def generate_ai_metadata(home, away, h_score, a_score, goals, competition_name):
    ...
    Goals: {', '.join([f"{g['player']} ({g['minute']}')" for g in goals]) if goals else 'No goals'}.
    Generate:
...
def process_match(fixture_id, home, away, h_score, a_score):
    ...
    result = supabase.table("matches").select("competition").eq("fixture_id", fixture_id).execute()
    ...
    if ai_meta:
        ...
        description = f"Watch the full match recap with cartoon animation! ⚽🎬\n\n#{comp_name.replace(' ', '')} #Highlights #Football"
